...
export default function Timeline({ className }: Props) {
  ...
  return (
    <div className={`relative flex flex-col min-w-xs pr-16 ${className}`}>
      <div className="absolute right-16 top-0 bottom-0 w-[1px] bg-custom-blue translate-x-[-7px]" />
      {items.map((item) => (
        <div
          key={item}
        ...
        >
          ...
          />
          <span
            className={`text-custom-blue text-sm text-right ${item === "Über mich" ? "!text-2xl font-semibold" : ""}`}
          >
            {item}{" "}
